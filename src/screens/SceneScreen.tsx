...
import { CinematicReturnMode, getLocalizedCinematicScene } from "../data/cinematicScenes";
...
export default function SceneScreen() {
    ...
    const { t: translateCinematic } = useTranslation(["cinematic"]);
    const { t: translateUi } = useTranslation(["ui"]);
    const scene = getLocalizedCinematicScene(sceneId, translateCinematic);
    ...
    if (!scene) {
        return (
            ...
            >
                <Typography>{translateUi("scene_not_found", { sceneId })}</Typography>
            </Box>
        );
    }
